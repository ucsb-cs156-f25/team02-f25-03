...
public class MenuItemReviewIT {
  ...
  public void an_admin_user_can_post_a_new_menuitemreview() throws Exception {
    ...
    MvcResult response =
        mockMvc
            .perform(
                post("/api/menuitemreview/post?itemId=2&reviewerEmail=janedoe@ucsb.edu&stars=5"
                        + "&dateReviewed=2025-10-28T12:00:00&comments=tasty")
                    .with(csrf()))
            .andExpect(status().isOk())
            .andReturn();

    // assert
    String expectedJson = mapper.writeValueAsString(menuItemReview2);
    String responseString = response.getResponse().getContentAsString();
    assertEquals(expectedJson, responseString);
  }
}
