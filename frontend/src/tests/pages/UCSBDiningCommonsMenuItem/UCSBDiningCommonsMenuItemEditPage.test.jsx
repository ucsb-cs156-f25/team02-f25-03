...
describe("UCSBDiningCommonsMenuItemEditPage tests", () => {
  ...
  describe("tests where backend is working normally", () => {
    ...
    test("Is populated with the data provided", async () => {
      ...
      expect(diningCommonsCodeField).toHaveValue("GSL");
      expect(nameField).toHaveValue("Pancakes");
      expect(stationField).toHaveValue("Morning");
    ...
    test("Changes when you click Update", async () => {
      ...
      fireEvent.change(diningCommonsCodeField, { target: { value: "BLS" } });
      fireEvent.change(nameField, { target: { value: "Chicken Wings" } });
      fireEvent.change(stationField, { target: { value: "Dark" } });

      fireEvent.click(submitButton);

      await waitFor(() => expect(mockToast).toBeCalled());
      expect(mockToast).toBeCalledWith(
        "UCSBDiningCommonsMenuItem Updated - id: 17 name: Chicken Wings",
      );
